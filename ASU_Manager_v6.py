    ...
    try:
        ...
        for line in r.stdout.splitlines():
            ...
            if '"' in l and ("интерфейс" in l.lower() or "Настройка" in l):
                ...
                ip = l.split()[-1]
                try: ipaddress.ip_address(ip); result.append({"name":name,"ip":ip})
                except: pass
    except: pass
    return result
...
class SubnetDlg(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        names = [f"{a['name']}  ({a['ip']})" for a in self.adapters] or ["Нет адаптеров"]
        self.v_adapter = ctk.StringVar(value=names[0])
    ...
    def _apply(self):
        ...
        try: idx = names.index(self.v_adapter.get())
...
class DeviceCard(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # Имя
        ctk.CTkLabel(top, text=d["name"], font=ctk.CTkFont(size=14,weight="bold"),
        ...
        for txt,col,cmd in [("✕",C["red"],   lambda:self.app.delete_device(d["id"])),
            ...
            ctk.CTkButton(top, text=txt, width=32, height=26, corner_radius=6,
        ...
        def ibtn(text, color, cmd):
            ctk.CTkButton(brf, text=text, width=36, height=30, corner_radius=8,
                          fg_color=C["bg3"], text_color=color,
                          hover_color=C["border"], font=ctk.CTkFont(size=13,weight="bold"),
...
class App(ctk.CTk):
    ...
    def _build_tab(self):
        ...
        # Название
        ctk.CTkLabel(self.tab, text="АСУ ТП",
                     font=ctk.CTkFont(size=10,weight="bold"),
